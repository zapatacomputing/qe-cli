...
if [[ "${QE_ON_LINUX-}" ]]; then
  if [[ "${QE_ON_64_BIT-}" ]]; then
    curl -s -S -L "https://github.com/zapatacomputing/qe-cli/releases/latest/download/qe-linux-amd64" -o /usr/local/bin/qe
  else
    curl -s -S -L "https://github.com/zapatacomputing/qe-cli/releases/latest/download/qe-linux-386" -o /usr/local/bin/qe
  fi
elif [[ "${QE_ON_DARWIN-}" ]]; then
  if [[ "${QE_ON_64_BIT-}" ]]; then
    curl -s -S -L "https://github.com/zapatacomputing/qe-cli/releases/latest/download/qe-darwin-amd64" -o /usr/local/bin/qe
  else
    curl -s -S -L "https://github.com/zapatacomputing/qe-cli/releases/latest/download/qe-darwin-386" -o /usr/local/bin/qe
  fi
else
